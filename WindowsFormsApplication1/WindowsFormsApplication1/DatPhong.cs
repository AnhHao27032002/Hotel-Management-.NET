...
namespace WindowsFormsApplication1
{
    public partial class DatPhong : Form
    {
        SqlConnection con;
        SqlCommand cmd;
        string str = "Data Source=ANHHAO\SQLSEVER2012;Initial Catalog=QUANLYKHACHSAN;Integrated Security=True";
        ...
        private void btn_search_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    cmd = con.CreateCommand();
                    cmd.CommandText = @"SELECT * FROM PHIEUDATPHONG WHERE IDPHIEUDATPHONG = '" + txt_maphieu_search.Text + "'";
                    adapter.SelectCommand = cmd;
                    table.Clear();
                    adapter.Fill(table);
                    dgv_phieudatphong.DataSource = table;
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    while (sqlReader.Read())
                                    {
                                        ...
                                        {
                                            cmd = con.CreateCommand();
                                            cmd.CommandText = @"SET DATEFORMAT DMY INSERT INTO PHIEUDATPHONG(IDPHIEUDATPHONG,IDKHACHHANG,IDPHONG,CHECKIN,SONGUOI) VALUES('" + txt_maphieu.Text.ToUpper() + @"','" + cbo_makh.Text + @"','" + cbo_maphong.Text + @"','" + dtp_checkin.Text + @"'," + txt_soluong.Text + ")";
                                            cmd.ExecuteNonQuery();
                                            cmd = con.CreateCommand();
                                            cmd.CommandText = @"UPDATE PHONG SET TRANGTHAI = N'Có Khách' WHERE (IDPHONG = N'" + cbo_maphong.Text + @"') SELECT * FROM PHONG";
                                            cmd.ExecuteNonQuery();
                                            LoadData();
                                        }
                                    }
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            try
            {
                cmd = con.CreateCommand();
                cmd.CommandText =
                    @"UPDATE PHONG SET TRANGTHAI = N'Trống' WHERE (IDPHONG = N'" + cbo_maphong.Text + @"')" +
                    @"SET DATEFORMAT DMY UPDATE PHIEUDATPHONG SET CHECKOUT='" + dtp_checkout.Text + "' WHERE IDPHIEUDATPHONG = '" + txt_maphieu.Text.ToUpper() + "'";
                cmd.ExecuteNonQuery();
                LoadData();
            }
            catch
        ...
        static double dongiadichvu, tongtiendichvu, x;
        ...
        private void btn_themdichvu_Click(object sender, EventArgs e)
        {
            try
            {
                SqlCommand sqlCmd = new SqlCommand("SELECT DICHVU.DONGIA, DICHVU.IDDICHVU FROM DICHVU WHERE TENDICHVU = N'" + cbo_dichvu.Text.Trim() + "'", con);
                con = new SqlConnection(str);
                con.Open();
                SqlDataReader sqlReader = sqlCmd.ExecuteReader();
                while (sqlReader.Read())
                {
                    ...
                    cmd.ExecuteNonQuery();
                    SqlCommand sqlCmd1 = new SqlCommand(@"SELECT TONGTIENDICHVU FROM CHITIETSUDUNG_DICHVU WHERE IDKHACHHANG = '" + cbo_makh.Text + "'", con);
                    SqlDataReader sqlReader1 = sqlCmd1.ExecuteReader();
                    while (sqlReader1.Read())
                    {
                        FormHoaDon.TongTienDichVu = double.Parse(sqlReader1[0].ToString());
                    }
                }
                sqlReader.Close();
            }
